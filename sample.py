...
from concept_conductor.models.custom_unet import CustomUnet
...
def sample(args):
    ...
    pipeline.unet = CustomUnet.from_pretrained(args.sd_ckpt, variant=variant, revision="fp16", torch_dtype=torch.float16, subfolder='unet').to(device)
    ...
    Path(args.image_outdir).mkdir(parents=True, exist_ok=True)
    sample_dir = str(Path(args.image_outdir) / Path(f"samples"))
    ...
    grid.save(str(Path(args.image_outdir) / Path(file_name)))
