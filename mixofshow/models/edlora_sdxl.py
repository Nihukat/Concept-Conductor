...
# Original code: https://github.com/TencentARC/Mix-of-Show/blob/main/mixofshow/models/edlora.py
...
class SDXL_EDLoRA_Control_AttnProcessor:
    r"""
...
class SDXL_EDLoRA_AttnProcessor:
    ...
    def change_forward(unet, count):
        for name, layer in unet.named_children():
            if layer.__class__.__name__ == 'Attention' and 'attn2' in name:
                layer.set_processor(SDXL_EDLoRA_AttnProcessor(count))
                count += 1
            else:
    ...
    def change_forward(unet, count, place_in_unet):
        for name, layer in unet.named_children():
            if layer.__class__.__name__ == 'Attention' and 'attn2' in name:  # only register controller for cross-attention
                layer.set_processor(SDXL_EDLoRA_Control_AttnProcessor(count, place_in_unet, controller))
                count += 1
            else:
