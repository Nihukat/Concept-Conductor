...
# Original code: https://github.com/TencentARC/Mix-of-Show/blob/main/mixofshow/data/pil_transform.py
...
class HumanResizeCropFinalV3(nn.Module):
    ...
    def forward(self, img, **kwargs):
        # step 1: short edge resize to self.size
        ...
        start_y = random.randint(0, self.size - new_height)
        start_x = random.randint(0, self.size - new_width)
    ...
    def forward(self, img, **kwargs):
        ...
        if random.random() < self.crop_p:
            if 'mask' in kwargs:
                ...
                img = self.random_crop(img)  # self.size
        else:
            # long edge resize
            img = F.resize(img, size=self.size - 1, max_size=self.size)
            if 'mask' in kwargs:
