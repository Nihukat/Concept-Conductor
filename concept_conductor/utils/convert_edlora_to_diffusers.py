...
def load_new_concept(pipe, new_concept_embedding, enable_edlora=True, tokenizer_idx=1, n_cross_attn_layers=16):
    if tokenizer_idx==1:
    ...
    for idx, (concept_name, concept_embedding) in enumerate(new_concept_embedding.items()):
        if enable_edlora:
            num_new_embedding = n_cross_attn_layers
        else:
            num_new_embedding = 1
...
def convert_edlora(pipe, state_dict, enable_edlora, alpha=0.6):
    ...
    if 'new_concept_embedding_2' in state_dict and len(state_dict['new_concept_embedding_2']) != 0:
        pipe, new_concept_cfg_2 = load_new_concept(pipe, state_dict['new_concept_embedding_2'], enable_edlora, tokenizer_idx=2)      
