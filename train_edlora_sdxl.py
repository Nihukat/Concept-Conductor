...
from mixofshow.pipelines.pipeline_edlora_sdxl import SDXLEDLoRAPipeline, StableDiffusionXLPipeline
from mixofshow.pipelines.trainer_edlora_sdxl import SDXLEDLoRATrainer
...
from test_edlora_sdxl import visual_validation
...
def train(root_path, args):
    ...
    # Load model
    EDLoRA_trainer = SDXLEDLoRATrainer(**opt['models'])
    ...
    original_embedding_2 = copy.deepcopy(accelerator.unwrap_model(EDLoRA_trainer).text_encoder_2.get_input_embeddings().weight)
    ...
    while global_step < opt['train']['total_iter']:
        with accelerator.accumulate(EDLoRA_trainer):
            ...
            accelerator.unwrap_model(EDLoRA_trainer).text_encoder_2.train()
            loss_dict = {}
            ...
            index_no_updates_2 = torch.arange(len(accelerator.unwrap_model(EDLoRA_trainer).tokenizer_2)) != -1
            if not stop_emb_update:
                for token_id in accelerator.unwrap_model(EDLoRA_trainer).get_all_concept_token_ids(tokenizer_idx=2):
                    index_no_updates_2[token_id] = False                    
        ...
        if accelerator.sync_gradients:
            ...
            token_embeds_2 = accelerator.unwrap_model(EDLoRA_trainer).text_encoder_2.get_input_embeddings().weight
            token_embeds_2.data[index_no_updates_2, :] = original_embedding_2.data[index_no_updates_2, :]
            token_embeds_2 = accelerator.unwrap_model(EDLoRA_trainer).text_encoder_2.get_input_embeddings().weight
            concept_token_ids_2 = accelerator.unwrap_model(EDLoRA_trainer).get_all_concept_token_ids(tokenizer_idx=2)
            loss_dict['Norm_mean_2'] = token_embeds_2[concept_token_ids_2].norm(dim=-1).mean()
            if stop_emb_update is False and float(loss_dict['Norm_mean_2']) >= train_opt.get('emb_norm_threshold_2', 1.5): 
                stop_emb_update = True
                original_embedding_2 = copy.deepcopy(accelerator.unwrap_model(EDLoRA_trainer).text_encoder_2.get_input_embeddings().weight)                
...
def save_and_validation(accelerator, opt, EDLoRA_trainer, val_dataloader, global_step, logger):
    ...
    if opt['val']['val_during_save']:
        ...
        for lora_alpha in opt['val']['alpha_list']:
            pipeclass = SDXLEDLoRAPipeline if enable_edlora else StableDiffusionXLPipeline

            # pipe = pipeclass.from_pretrained(opt['models']['pretrained_path'], variant="fp16",
        ...
        EDLoRA_trainer.to(device) 


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    # parser.add_argument('-opt', type=str, default='options/train/EDLoRA/real/corgi_dog.yml')
    parser.add_argument('-opt', type=str, default='configs/edlora/train_sdxl/chow_dog.yml')
